...
from antd.ant   import RequestDataPage, DATA_PAGE_COMMON_MANUFACTURERS_INFO, DATA_PAGE_REQUEST_DATA, DEVICE_TYPE
...
def listen_for_broadcast_then_request_data_page(session, device_type):
        network = session.networks[0]
        if device_type == DEVICE_TYPE.GPS:
            network.set_key("\xa8\xa4\x23\xb9\xf5\x5e\x63\xc1")
        elif device_type in [DEVICE_TYPE.HRM,DEVICE_TYPE.SDM]:
        ...
        if device_type == DEVICE_TYPE.GPS:
            channel.set_id(device_number=0, device_type_id=DEVICE_TYPE.GPS, trans_type=0)
            channel.set_period(0x4000)
            channel.set_rf_freq(50)
        elif device_type in [DEVICE_TYPE.HRM,DEVICE_TYPE.SDM]:
            if device_type == DEVICE_TYPE.HRM:
                channel.set_id(device_number=0, device_type_id=DEVICE_TYPE.HRM, trans_type=0)
                channel.set_period(8070)
            elif device_type == DEVICE_TYPE.SDM:
                channel.set_id(device_number=0, device_type_id=DEVICE_TYPE.SDM, trans_type=0)
                channel.set_period(8134)
...
def main():
    ...
    core = ant.Core(dev)
    if 1:
        print ">>>>>>>>>>>>>>>>  Garmin Footpod/SDM >>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
        session = ant.Session(core)
        try:
            listen_for_broadcast_then_request_data_page(session, device_type=DEVICE_TYPE.SDM)
    ...
    if 0:
        ...
        session = ant.Session(core)
        try:
            listen_for_broadcast_then_request_data_page(session, device_type=DEVICE_TYPE.HRM)
        except AntTimeoutError:
            _LOG.warning("Timed out - Device not present?")
        finally:
            try: session.close()
    ...
    if 0:
        ...
        session = ant.Session(core)
        try:
            listen_for_broadcast_then_request_data_page(session, device_type=DEVICE_TYPE.GPS)
        except AntTimeoutError:
            _LOG.warning("Timed out - Device not present?")
        finally:
            try: session.close()
